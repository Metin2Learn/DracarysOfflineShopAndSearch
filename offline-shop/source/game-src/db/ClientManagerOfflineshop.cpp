...
void CClientManager::CreateOfflineShop(TOfflineShop* p)
{
	...
	for (DWORD i = 0; i < OFFLINE_SHOP_HOST_ITEM_MAX_NUM; ++i)
	{
		...
		for(BYTE k=0;k<ITEM_SOCKET_MAX_NUM;++k)
		{
			iLen += snprintf(szColumns + iLen, sizeof(szColumns) - iLen, ",socket%d",k);
			iUpdateLen += snprintf(szValues + iUpdateLen, sizeof(szValues) - iUpdateLen, ",%ld", offlineshop->items[i].alSockets[k]);
		}

		for(BYTE k=0;k<ITEM_ATTRIBUTE_MAX_NUM;++k)
		{
			iLen += snprintf(szColumns + iLen, sizeof(szColumns) - iLen, ", attrtype%d, attrvalue%d",k ,k);
			iUpdateLen += snprintf(szValues + iUpdateLen, sizeof(szValues) - iUpdateLen, ",%d,%d",
			offlineshop->items[i].aAttr[k].bType, offlineshop->items[i].aAttr[k].sValue);
		}
...
void CClientManager::AddItem(OFFLINE_SHOP_ITEM* item)
{
	...
	{
		iLen += snprintf(szColumns + iLen, sizeof(szColumns) - iLen, ",socket%d", k);
		iUpdateLen += snprintf(szValues + iUpdateLen, sizeof(szValues) - iUpdateLen, ",%ld", newitem.alSockets[k]);
	}
	for(BYTE k=0;k<ITEM_ATTRIBUTE_MAX_NUM;++k)
	{
		iLen += snprintf(szColumns + iLen, sizeof(szColumns) - iLen, ", attrtype%d, attrvalue%d",k,k);
		iUpdateLen += snprintf(szValues + iUpdateLen, sizeof(szValues) - iUpdateLen, ",%d,%d",newitem.aAttr[k].bType, newitem.aAttr[k].sValue);
	}
...
void CClientManager::BuyItem(TOfflineShopBuy* buyItem)
{
	...
	for (DWORD i = 0; i < OFFLINE_SHOP_HOST_ITEM_MAX_NUM; i++)
	{
		...
		for (BYTE j = 0; j < ITEM_ATTRIBUTE_MAX_NUM; ++j)
			iLen += snprintf(szQuery + iLen, sizeof(szQuery) - iLen, ",attrtype%d,attrvalue%d", j,j);
		iLen += snprintf(szQuery + iLen, sizeof(szQuery) - iLen, ") VALUES(%u,'%s','%s',%d,%d,%lld,%u,%d", item.owner_id, buyItem->customer_name, buf, item.vnum, item.count, item.price,item.id, item.pos);
#ifdef ENABLE_CHANGELOOK_SYSTEM
		iLen += snprintf(szQuery + iLen, sizeof(szQuery) - iLen, ", %d", item.transmutation);
#endif
		for (BYTE j = 0; j < ITEM_SOCKET_MAX_NUM; ++j)
