...
#############################################################
	...
	def UpdateItemsList(self):
		if localeInfo.IsARABIC():
			input_text = self.itemNameSearch.GetText()
		else:
			input_text = self.itemNameSearch.GetText().lower()
		...
		for i, itemVnum in enumerate(items_list, start=1):
			...
			if localeInfo.IsARABIC():
				listItemName = item.GetItemName()
			else:
				listItemName = item.GetItemName().lower()
			if input_len > len(listItemName):
				continue
			#elif len(listItemName) > input_len+3:
			#	continue
			isLowRefine = False
			if listItemName.find("+") != -1:
				for j in xrange(9):
					if listItemName.find("+%d"%j) != -1:
						isLowRefine = True
						continue
			if isLowRefine:
				continue
			if listItemName[:input_len] == input_text:
				self.selectedItem = itemVnum
				self.itemNameSearchText.SetText(listItemName)
				return

		skill_list = self.getSkillNames(str(input_text))
		for skill in skill_list:
			if input_len > len(skill):
				continue
			elif len(skill) > input_len*2:
				continue
			if skill[:input_len] == input_text:
